...
class APIImpl:
    ...
    @staticmethod
    def _get_edge_options(headless, maximize, options):
        result = EdgeOptions() if options is None else options
        ...
        # 禁止出现正在被控制的提示
        result.add_argument("--disable-blink-features")
        result.add_argument("--disable-blink-features=AutomationControlled")
        if headless:
    ...
    def write_impl(self, text, into=None):
        if into is not None:
            from plugins.helium import GUIElement
    ...
    def _unwrap_clickable_element(self, elt):
        from plugins.helium import HTMLElement, Point
        offset = None
    ...
    def select_impl(self, combo_box, value):
        from plugins.helium import ComboBox
    ...
    def attach_file_impl(self, file_path, to=None):
        from plugins.helium import Point
        driver = self.require_driver()
        if to is None:
            to = FileInput(driver)
    ...
    def switch_to_impl(self, window):
        driver = self.require_driver()
        from plugins.helium import Window
    ...
    def highlight_impl(self, element):
        driver = self.require_driver()
        from plugins.helium import HTMLElement, Text
...
class GUIElementImpl:
    ...
    def perform(self, action):
        from plugins.helium import Config
...
class HTMLElementImpl(GUIElementImpl):
    ...
    def _unwrap_element(self, element):
        ...
        from plugins.helium import HTMLElement
        if isinstance(element, HTMLElement):
...
class LabelledElement(HTMLElementImpl):
    ...
    @staticmethod
    def _get_labels_with_explicit_elts(all_elts, labels):
        for label in labels:
